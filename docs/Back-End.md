...
- `nn-classifier/`: Python neural network scripts and models
- `training_data/`: Collected images for future training
- `bin/`: Compiled back-end binary
- `backend.log`: Request and error logs (with NZ timestamps)
- `install-cricket-ready-backend.sh`: Automated install and setup script
- `uninstall-cricket-ready-backend.sh`: Automated uninstall script
...
**Purpose:**
Classifier a cricket ball image as `match-ready` or `not-match-ready`.
...
**Response:**
- JSON: `{ "prediction": "match-ready" | "not-match-ready", "confidence": float }`
...
**Purpose:**
Collects labeled images for future model training.

**Request:**
- Multipart form-data
- Fields:`image` (image file), `label` (string: "match-ready" or "not-match-ready")

**Response:**
- JSON: `{ "status": "success", "filename": "...", "label": "...", "request_id": ... }`

---

## Request Loggin

- All requests are logged to `backend.log` with NZ timestamps.
- Each request is tagged with a unique `request_id` for traceability.

---

## Model Execution

- The Rust back-end saves uploaded images to a temporary file.
- It calls the Python script `nn-classifier/predict.py` via subprocess.
- The Python script loads an ensemble of PyTorch models and returns a prediction and confidence score.
...
- Images submitted via `/train` are saved in `training_data/match-ready/` or `training_data/not-match-ready/` based on the label.
- Model retraining is performed manually by running `nn-classifier/train.py` with the collected images.
